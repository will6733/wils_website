...
export function GlowCard({ children, className = "", glowColor = "blue", width, height }: GlowCardProps) {
  ...
  const inlineStyles: React.CSSProperties & Record<string, string | number> = {
    ...
    backgroundImage: `radial-gradient(var(--spotlight-size) var(--spotlight-size) at calc(var(--x,0)*1px) calc(var(--y,0)*1px),hsl(var(--hue,210) calc(var(--saturation,100)*1%) calc(var(--lightness,70)*1%)/var(--bg-spot-opacity,0.12)),transparent)`,
